 ...
 * The mapping of these actions to axes depends on the up axis, which defaults to `Y` (but can be changed with the {@link Index.setUpAxis | setUpAxis() method}):
 *
 ...
 * This can be configured through {@link Index.translateAlong | translateAlong property}.
 * It can also be overwritten by providing the component name to the {@link Index.do | do() method}, see {@link https://github.com/nytimes/three-story-controls/blob/main/src/controlschemes/ThreeDOFControls.ts#L96 | ThreeDOFControls implementation} for an example.
 *
 * To move the rig along a specified path, use the {@link Index.setAnimationClip | setAnimationClip() method},
 ...
 */
export class Index extends EventDispatcher {
	...
	private animationClip: AnimationClip | undefined;
	...
	constructor( camera: Camera, scene: Scene ) {
		...
		this.body.rotation.order = this.getRotationOrder() as EulerOrder;
		this.head.rotation.order = this.getRotationOrder() as EulerOrder;
		this.eyes.rotation.order = this.getRotationOrder() as EulerOrder;
